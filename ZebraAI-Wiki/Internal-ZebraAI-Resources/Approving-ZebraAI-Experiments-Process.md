...
Checklist](ZebraAI-Experiment-Approvers-Checklist.md).
...
1.  Approver logs in to the ZebraAI site and goes to [Admin\\Approve
    Experiments](https://zebra-ai-web-prd.ait.microsoft.com/admin/approveexperiments).

2.  Review the list of experiments that are waiting for approval. You
    can click on the icon under Review for each experiment to see more
    details.

3.  For each experiment, decide whether to approve or reject it. You use
    the [ZebraAI Experiment Approvers Checklist](ZebraAI-Experiment-Approvers-Checklist.md).
    to evaluate the experiment.

4.  If you approve the experiment, click on the Approve button. This
    will move the experiment to the Approved filter and notify the
    creator of the approval.

5.  If you reject the experiment, provide a reason for the rejection
...
6.  Repeat steps 4 to 6 for each experiment waiting for approval.

## Prompt Review Tool
In August 2026, the ZebraAI team introduced a new tool to make reviewing prompts easier. To use this tool, click the blue "Review Prompt" button under the experiment prompt. 

![Prompt Reviewer](/wiki/imgs/prompt_reviewer_1.png)

This will open a pop-up that formats the prompt in a much more easily readable way, with accordion sections for each prompt message and warnings for repeated substitution parameters. 

![Prompt Reviewer](/wiki/imgs/prompt_reviewer_2.png)

You can open each section to review the prompt in smaller chunks. 

![Prompt Reviewer](/wiki/imgs/prompt_reviewer3.png)
...
1.  The approver or the creator who notices the problem with the
    ...
    team](https://aka.ms/ZebraAIFeedback).
